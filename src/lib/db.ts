 ...
 *
 * Production hosts (Vercel, Grok App Builder) refuse to boot without DATABASE_URL
 * so they never silently serve an ephemeral in-memory database.
 */
...
function assertProductionDatabase() {
  const hosted =
    ...
    (typeof process !== "undefined" && Boolean(process.env.GROK_PROJECT_ID));
  if (hosted && !databaseUrl) {
    throw new Error(
      "DATABASE_URL is required in production. Embedded Postgres is preview-only.",
    );
  }
}
